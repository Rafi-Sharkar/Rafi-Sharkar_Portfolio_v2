...
const styleByColor = {
  'accent-cyan': { bgClass: 'bg-accent-cyan/10', iconClass: 'text-accent-cyan' },
  'accent-purple': { bgClass: 'bg-accent-purple/10', iconClass: 'text-accent-purple' },
  'accent-pink': { bgClass: 'bg-accent-pink/10', iconClass: 'text-accent-pink' },
  'accent-emerald': { bgClass: 'bg-accent-emerald/10', iconClass: 'text-accent-emerald' },
};

// Maps a gradient string (e.g. "from-green-400 to-emerald-600") to an icon tint.
// Falls back to accent-cyan when the value is something we don't recognise.
const gradientToIconClass = (gradient) => {
  if (!gradient) return 'text-accent-cyan';
  const first = gradient.split(' ')[0] || '';
  if (first.includes('green') || first.includes('emerald')) return 'text-green-400';
  if (first.includes('blue') || first.includes('indigo') || first.includes('cyan')) return 'text-blue-400';
  if (first.includes('pink') || first.includes('rose')) return 'text-pink-400';
  if (first.includes('purple') || first.includes('violet') || first.includes('fuchsia')) return 'text-purple-400';
  if (first.includes('yellow') || first.includes('amber') || first.includes('orange')) return 'text-yellow-400';
  return 'text-accent-cyan';
};

const resolveStyles = (item) => {
  const known = styleByColor[item.color];
  if (known) return known;
  // Color might be a Tailwind gradient like "from-green-400 to-emerald-600".
  return {
    bgClass: 'bg-accent-cyan/10',
    iconClass: gradientToIconClass(item.color),
  };
};
...
const normalizeContactHref = (item) => {
  ...
  if (cardType === 'phone') {
    const source = rawHref || value;
    if (!source) return '';
    if (/^tel:/i.test(source)) return source;
...
function ContactCard({ item }) {
  ...
  const styles = resolveStyles(item);
...
export default function ContactForm({ showFooter = true }) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20">
            <motion.div
              ...
              className="space-y-6"
            >
              <h3 className="text-2xl font-bold text-white mb-8">{contact.title}</h3>
