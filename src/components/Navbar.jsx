  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? 'py-2 glass-dark shadow-lg shadow-dark-900/50' : 'py-4 bg-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link, i) => (
              <motion.div key={link.path} custom={i} initial="hidden" animate="visible" variants={linkVariants}>
                <Link
                  href={link.path}
                  className={`relative px-4 py-2 text-base font-medium transition-all duration-300 rounded-full group ${
                    pathname === link.path ? 'text-accent-cyan' : 'text-gray-300 hover:text-white'
                  }`}
                >
                  {link.label}
                  {pathname === link.path && (
          ...
          <>
            ...
            >
              ...
              <div className="absolute bottom-8 left-8 right-8">
                ...
                <p className="text-center text-xs text-gray-500 mt-4">© 2025 Rafi Sharkar</p>
              </div>
            </motion.div>
          </>
        )}
